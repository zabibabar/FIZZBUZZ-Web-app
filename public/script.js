...
function loadScore() {
   ...
   get("http://127.0.0.1:3000/"+username).then(function(response){ 
      if(response.status == 200){
         ...
         document.getElementById("hiddenCounter").innerHTML = parseInt(response.data.score);//Save the hidden counter
      }   
      else
         document.getElementById("numDisplay").innerHTML = "Click increment to begin!";   
   });
}
...
function increment () {
   ...
   let currVal = parseInt(document.getElementById("hiddenCounter").innerHTML,10);  
